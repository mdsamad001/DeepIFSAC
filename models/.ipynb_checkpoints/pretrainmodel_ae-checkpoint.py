...
class AE(nn.Module):
    ...
    def __init__(self, x_dim, z_dim=5) -> None:
        super().__init__()

        self.x_dim  = x_dim # should be n*d
        self.z_dim = z_dim

        z1_dim, z2_dim = 256, 32

        self.encoder = nn.Sequential(
            
            nn.Linear(x_dim, z1_dim),
            nn.ReLU(),
            nn.Linear(z1_dim, z2_dim),
            nn.ReLU(),
            nn.Linear(z2_dim, z_dim),
            nn.ReLU(),
        )

        self.decoder = nn.Sequential(
            nn.Linear(z_dim, z2_dim),
            nn.ReLU(),
    ...
    def forward(self, X):
        ...
        Z = self.encoder(X1)
        X1_hat = self.decoder(Z)
        X_hat = rearrange(X1_hat, 'b (n d) -> b n d', n=n)
...
class SAINT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.mlpfory = simple_MLP([dim ,1000, y_dim]) # mlp on CLS for predicting classification label or regression value
        ...
        self.fe_ae = AE(dim*(self.num_continuous+self.num_categories), 256)
    ...
    def forward(self, x_categ, x_cont):
        ...
        z, x_hat = self.fe_ae(x)

        cat_outs = self.mlp1(x_hat[:,:self.num_categories,:])
        con_outs = self.mlp2(x_hat[:,self.num_categories:,:])

        return cat_outs, con_outs, z, x, x_hat